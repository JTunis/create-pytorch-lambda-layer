...
do
    case $arg in
        ...
        *) echo "ERROR: Invalid argument ${arg}" && exit;;
    esac
done
...
case ${PYTHON_VERSION} in
    ...
    "") echo "ERROR: No Python version specified" && exit;;
    *) echo "ERROR: Invalid Python version. Expected versions include: 2.7, 3.5, 3.6, 3.7, 3.8, 3.9" && exit;;
esac

# build torch wheel URL
case ${TORCH_VERSION} in
    "") "ERROR: No Torch version specified" && exit;;
...
${TORCHVISION_WHEEL} \
${TORCHAUDIO_WHEEL} \
-t python/lib/python${PYTHON_VERSION}/site-packages; exit" || exit

# remove extraneous files and directories
cd "python/lib/python${PYTHON_VERSION}/site-packages" || exit
